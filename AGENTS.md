...
## コードレビュー＆Skill化ポリシー (Code Review & Skill Distillation)

- CodeRabbitや人間からのコードレビュー指摘を受けた場合、単に該当箇所を修正するだけで終わらせないこと。
- 指摘の根本的な原因（早期リターンの欠落、定数化の漏れ、フォールバックUIの欠如など）を分析し、汎用的なベストプラクティスを言語化すること。
- 抽出したベストプラクティスは `.agent/skills/` 配下のドキュメント（例: `coding-guidelines.md`）にSkillとして追記・蓄積し、再発防止のルールとして運用すること。